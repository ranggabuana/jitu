<x-layout>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6 max-w-2xl">
        <form action="{{ route('opd.update', $opd->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="mb-6">
                ...
                <input type="text" id="kode_opd" name="kode_opd" value="{{ old('kode_opd', $opd->kode_opd) }}"
            ...
            <div class="mb-6">
                ...
                @if($opd->gambar_tte)
                    <div class="mb-3">
                        <img src="{{ asset('storage/' . $opd->gambar_tte) }}" alt="Gambar TTE {{ $opd->nama_opd }}" class="max-h-32 rounded-lg border border-gray-300 dark:border-gray-600">
                    </div>
                @endif
                ...
                <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Format yang didukung: JPEG, PNG, JPG, GIF, SVG. Ukuran maksimal: 2MB.<br>Biarkan kosong jika tidak ingin mengubah gambar.</p>
                @error('gambar_tte')
                    <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                @enderror
            </div>
