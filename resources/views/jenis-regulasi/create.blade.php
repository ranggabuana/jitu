<x-layout>
    <x-slot:title>Tambah Jenis Regulasi</x-slot:title>
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-gray-800 dark:text-white">Tambah Jenis Regulasi</h1>
        <p class="text-gray-600 dark:text-gray-400 mt-1">Tambah jenis regulasi baru</p>
    </div>

    <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6 max-w-3xl">
        <form action="{{ route('jenis-regulasi.store') }}" method="POST">
            @csrf
            ...
            <div class="mb-6">
                ...
                <input type="text" id="nama_jenis" name="nama_jenis" value="{{ old('nama_jenis') }}"
            ...
            <div class="mb-6">
                ...
                <textarea id="deskripsi" name="deskripsi" rows="4"
                    ...
                    placeholder="Deskripsi singkat tentang jenis regulasi ini">{{ old('deskripsi') }}</textarea>
                @error('deskripsi')
                    <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                @enderror
            </div>
