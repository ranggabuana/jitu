<x-layout>
    <x-slot:title>Edit Regulasi</x-slot:title>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
        <h2 class="text-xl font-bold text-gray-800 dark:text-white mb-6">Edit Regulasi</h2>

        <form action="{{ route('regulasi.update', $regulasi->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <input type="text" name="nama_regulasi" id="nama_regulasi" value="{{ old('nama_regulasi', $regulasi->nama_regulasi) }}"
            ...
            <div>
                ...
                <input type="text" name="slug" id="slug" value="{{ old('slug', $regulasi->slug) }}"
            ...
            <div>
                ...
                <div class="mb-2">
                    <p class="text-sm text-gray-600 dark:text-gray-400">
                        <i class="mdi mdi-file mr-1"></i>File saat ini: 
                        <a href="{{ route('regulasi.download', $regulasi->id) }}" class="text-blue-600 hover:underline" target="_blank">
                            {{ pathinfo($regulasi->file_regulasi, PATHINFO_FILENAME) }}
                        </a>
                ...
                <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">Upload file baru untuk mengganti file lama. Format: PDF, DOC, DOCX, XLS, XLSX, PPT, PPTX, ZIP, RAR (Max: 10MB)</p>
                @error('file_regulasi')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <textarea name="deskripsi" id="deskripsi" rows="4"
                    ...
                    placeholder="Deskripsi singkat tentang regulasi ini...">{{ old('deskripsi', $regulasi->deskripsi) }}</textarea>
                @error('deskripsi')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <select name="status" id="status"
                    ...
                    <option value="tidak_aktif" {{ old('status', $regulasi->status) === 'tidak_aktif' ? 'selected' : '' }}>Tidak Aktif</option>
                </select>
                @error('status')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
